...
- Chat = Claude Code reale: streaming con **rendering markdown** (`webview/ui/Markdown.tsx` → react-markdown+remark-gfm: grassetto/code/liste/heading/tabelle/blockquote; stili in `.ac-md`), **animazioni** (dots "pensando", caret lampeggiante, shimmer "sta lavorando", pulse-ring su domande/approvazioni), **tool con diff** inline (Edit/Write/MultiEdit), **modal di approvazione** (1/2/3), **AskUserQuestion** (QuestionModal; la risposta viene **rieccheggiata in chat** e salvata nel transcript), **Plan mode** (PlanModal → Approva/Continua), **@-mention** file, allega **immagini** (drag-drop/paste/picker → content block base64), **Figma** (via MCP), mode/model/thinking/effort, **stop** (send→quadrato), interrupt.
- Preview/Design/Code: preview live + **select component stile Cursor** (→ apre un **modale prompt per-componente** `ComponentPromptModal`: Edit/Prompt, Builder, classi+sorgente, Applica), device picker, fullscreen, settings preview, preview **collassabile**; **Code view = mini-IDE** (vedi limite #4) con accento **verde** anche nel tab switcher; splitter trascinabile, Preview mode = full-screen.
...
- **Auto-update via GitHub Releases**: `src/update/checkForUpdate.ts` interroga `repos/elyas-tirit/Agent-Code/releases/latest` ogni 6h (throttle in `globalState`), confronta semver con `package.json`, e se c'è una nuova versione propone toast **Aggiorna / Più tardi / Salta questa versione**. "Aggiorna" → download streaming del `.vsix` con progress bar → `workbench.extensions.installExtension` → prompt reload. Comando manuale "Agent Code: Check for Updates" + setting `agentCode.checkForUpdates`. Workflow CI (`.github/workflows/release.yml`) triggera sul push a `main` quando `package.json` cambia: legge la version, se `v<version>` non esiste ancora fa `npm ci → typecheck → test → package → gh release create` con il `.vsix` allegato. **Net effect del release flow**: bump `package.json` version + commit + push su main → entro 6h tutti gli utenti installati vedono il prompt di update.
- **Patch notes on update** ("What's New" panel): dopo un aggiornamento, alla prossima activate compare un webview panel custom (`src/panels/ChangelogPanel.ts` + `webview/views/changelog/`) che mostra le novità di ogni versione saltata. Contenuto autorato in `media/changelogs/<version>.json` (tipi `Changelog`/`ChangelogSection`/`ChangelogVisual` in `protocol.ts`); il loader (`src/changelog/loader.ts`) raccoglie tutti i JSON con `version > lastSeen && <= current`. Layout designer-friendly (highlights chip + section card con accent colors + visuals "live" come `ToastMock`/`PaletteMock`/`SizeChart` invece di PNG — scalano col tema) e dettagli tecnici dietro un `<details>` "For the curious". High-water mark in `globalState[agentCode.changelog.lastSeenVersion]`. Distingue **fresh install** (esiste `agents.json` o no?) da **upgrade**: fresh install seed silenzioso, upgrade mostra il panel. Setting `agentCode.showPatchNotesOnUpdate` (default true) + bottone "Don't show on update" sul panel + comando manuale "Agent Code: What's New".
...
4. **Code view** = mini-IDE (activity rail, Explorer+search, tree con badge file-type, **multi-tab**, **syntax highlighting** via prism-react-renderer, accento **verde**, **skeleton di loading**, **selezione righe→AI**: click/shift/drag sul gutter → range evidenziato → "Chiedi all'AI" allega `file:Lx-Ly` + snippet al contesto chat). Resta **read-only** (non editor) e senza refresh live legato alle modifiche dell'agente. `webview/views/design/CodeView.tsx` + `ui/FileIcon.tsx`.
...
- **Injection inline di script**: `picker.js` (e qualsiasi JS iniettato come `<script>…</script>` dal `PreviewProxy`) NON deve contenere il literal `</script>` (nemmeno in un commento) — chiude il tag in anticipo e il resto finisce renderizzato come **testo** nella pagina. Il proxy ora escapa `</script>`→`<\/script>` per sicurezza; tienilo.
- **Modali chat = `FloatingPanel`** (approval/question/plan): trascinabili dall'header, **minimizzabili** in un chip (riapribili), **senza backdrop bloccante** (`pointer-events-none` sul wrapper) così vedi/interagisci dietro. Il minimize è gestito da `DesignWorkspace` con un `Set<id>`.
- **Claude Code indicizza le sessioni per cartella** (`~/.claude/projects/<cwd-encoded>/<sessionId>.jsonl`). Il **resume DEVE usare la cwd originale dell'agente**, non la cartella workspace corrente — altrimenti "No conversation found with session ID". Per questo `PersistedAgent.cwd` e `agentCwds` esistono; `getOrWakeSession` fa resume con `agentCwds.get(id) ?? this.cwd`. C'è anche una rete di sicurezza in `ClaudeSession.run`: se il resume fallisce, riparte pulito (una volta) invece di mostrare errore.
